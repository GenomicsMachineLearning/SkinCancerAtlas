...
```{r}
outdir <- "/Volumes/SciDrive/Archive/SCC/1_sandbox/20220722_GeoMxNanostring/1_GeneOntology/2_GO/outdir_top100/"

sampleID <- "RNA"

all <- read.delim("/Volumes/SciDrive/Archive/SCC/1_sandbox/20220722_GeoMxNanostring/1_GeneOntology/1_processGeneLists/outdir/DEgenes_top100.txt")

universe <- read.delim("/Volumes/SciDrive/Archive/SCC/1_sandbox/20220722_GeoMxNanostring/1_GeneOntology/1_processGeneLists/outdir/universe.txt")
```
...
DE <- all[DE$genes %in% names(universe_entrez),]

prep_IDs <- function(list_to_test) {
  geneList <- pull(DE %>% dplyr::filter(sample == list_to_test), "genes")
  myIDs = mapIds(org.Hs.eg.db, column = "ENTREZID", keytype = "SYMBOL", keys = geneList)
  myIDs = myIDs[!(is.na(myIDs))]
  return(myIDs)
}
all_entrez <- lapply(unique(DE$sample), prep_IDs)
names(all_entrez) <- unique(DE$sample)
```
